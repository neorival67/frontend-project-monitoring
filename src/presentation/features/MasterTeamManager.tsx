...
import { useMasterTeams, useCreateMasterTeam, useUpdateMasterTeam } from "@/use-cases/hooks/useMasterTeam";
import { DeleteModalTeam } from "@/presentation/components/DeleteModalTeam";
  ...
  return (
    <div className="w-full space-y-6 pb-10">
      <div className="flex justify-between items-end mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Master Tim</h1>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-[0_2px_10px_-3px_rgba(0,0,0,0.5)] overflow-hidden">
        {isLoading ? (
            ...
            <tbody className="divide-y divide-slate-800">
              {teams.map((team: any) => (
                <tr key={team.id} className="hover:bg-slate-800/50 transition-colors">
                  <td className="p-4 pl-6 font-semibold text-slate-200">{team.name}</td>
                  <td className="p-4 text-sm text-slate-400">{team.description}</td>
                  <td className="p-4 flex items-center justify-center gap-2">
                    <button onClick={() => handleOpenModal(team)} className="p-2 text-blue-400 hover:bg-blue-900/30 rounded-lg transition-colors" title="Edit">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                    </button>
                    <button onClick={() => handleDeleteClick(team.id)} className="p-2 text-rose-400 hover:bg-rose-900/30 rounded-lg transition-colors" title="Hapus">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-800/50">
              <h2 className="font-bold text-slate-200">{editingTeam ? "Edit Tim" : "Tambah Tim Baru"}</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-sm text-slate-200 outline-none focus:border-blue-500"
                />
              </div>
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-sm text-slate-200 outline-none focus:border-blue-500 h-24 resize-none"
                ></textarea>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-slate-800 flex justify-end gap-3 bg-slate-800/50">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm font-medium text-slate-300 bg-slate-800 border border-slate-700 rounded-lg hover:bg-slate-700">Batal</button>
